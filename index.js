...
// Debug: log loaded modules
console.log('[discord.js] Loaded components:', Object.keys(require('discord.js')));

// sheet helpers
const { fetchAllMasterRows, fetchNewBets, markRowSend } = require('./sheets');
// DB-backed user stakes
const userService = require('./services/userService');
console.log('[api] userService methods:', Object.keys(userService));
...
app.get('/api/user-stakes', async (req, res) => {
  console.log('[api] GET /api/user-stakes', req.query);
  const authHeader = req.headers.authorization || '';
  const token      = authHeader.replace(/^Bearer\s+/, '');
  if (token !== WEBHOOK_KEY) {
    console.error('[api] Unauthorized token', token);
  ...
  try {
    ...
    console.log('[api] returning stakes count:', stakes.length);
    return res.json(stakes);
  } catch (err) {
    console.error('[api] Error in user-stakes:', err);
...
app.post('/discord-role', async (req, res) => {
  console.log('[api] POST /discord-role', req.body);
  const authHeader = req.headers.authorization || '';
  const token      = authHeader.replace(/^Bearer\s+/, '');
  ...
  if (!action || !discord_id || !role_id || !guild_id) {
    console.error('[api] Missing fields in discord-role');
  ...
  try {
    ...
    console.log('[api] Role sync success', action, discord_id);
    return res.status(200).send('OK');
  } catch (err) {
    console.error('[api] Discord role sync error:', err);
...
async function processNewBets() {
  console.log('[bot] processNewBets start');
  try {
    const allRows = await fetchAllMasterRows();
    console.log('[bot] fetched rows count:', allRows.length);
    for (let i = 1; i < allRows.length; i++) {
      const row = allRows[i];
      if (row[9] === 'S') {
        console.log('[bot] New bet pending at row:', i);
        ...
        console.log('[bot] Posting embed for betId:', betId);
    ...
    console.error('[bot] Error in processNewBets:', err);
  }
}
  ...
  try {
    if (interaction.isButton() && interaction.customId.startsWith('stakeModal_')) {
      ...
      if (user.staking_mode === 'flat') {
        ...
        if (stake*(odds-1) < user.stw_amount) stake++;
        recommended = stake;
      } else {
    ...
    if (interaction.type === InteractionType.ModalSubmit && interaction.customId.startsWith('stakeModalSubmit_')) {
      console.log('[bot] Modal submitted:', interaction.customId);
      const betId     = interaction.customId.split('_')[1];
      const discordId = interaction.user.id;
